...
function Home() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const foundTodayClasses: ClassData[] =
        await axiosClassRequest.getTodayClasses();
  ...
  return (
    <ContentArea>
      ...
      <ContentContainer>
        ...
        <ClassCardArea>
          {todayClasses.length === 0 ? (<Loading />) : (todayClasses.map((item: ClassData) => (
              <HomeClassCard classContent={item} />
            ))
          )}
        </ClassCardArea>
      </ContentContainer>
